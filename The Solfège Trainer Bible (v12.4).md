# The Solfège Trainer Bible (v12.4)
**Status:** Production - Phase 1 & Phase 2 Complete | **Last Updated:** July 31, 2026
...
* **Input States:**
    ...
    * **Transcription (Levels 3–6, 9–12):** Sequential placeholders (`__ __`). Requires ENTER to submit.
...
* **Level 1 & Level 7 Expansion:** Correct answers reveal the full resolution (e.g., `ti` reveals `ti do`; `fi` reveals `fi sol`).
...
### Phase 3–5: Dyads & Stacks (Future Roadmap)
...
**July 31 Release Notes (v12.4):**
* **Directional Enharmonic Engine:** Implemented accurate functional spelling rules for all chromatic pitches.
* **Phase 2 Complete:** Implemented Levels 7 through 12 with exact note count enforcement.
* **Audio Clamping:** Added strict range bounds ($40 \le \text{MIDI} \le 81$) preventing out-of-range silent note drops.
* **Weighted Spiral Learning:** Boosted current level selection weight (40% boost) during cumulative pool generation.
* **UX Fixes:** Added `K` cadence keybind, Caps Lock ignore, instant panic on level navigation, and duplicate sequence guards.
