...
function M.getPreferredName(pitch, context)
    local map = {
        ...
        [5]="fa", [6]="fi", [7]="sol", [8]="le", [9]="la", 
        [10]="te", [11]="ti"
    }
    return map[(pitch % 12 + 12) % 12] or "???"
end
                ...
                if lastMIDI then
                    ...
                    while (pitch == lastMIDI or math.abs(pitch - lastMIDI) > 12) and attempts < 15 do
                        if pitch == lastMIDI then
                            -- It's a duplicate. Re-roll from the strictly diatonic table.
                            pitch = diatonicPitches[math.random(#diatonicPitches)]
                        else
                            -- It's a giant leap. Move it by octaves (always preserves diatonicity).
