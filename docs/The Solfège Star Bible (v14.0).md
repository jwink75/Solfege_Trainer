...
**Status:** Production - Solfège Star Master Bible | Full Syllabus Granularity Restored, Agnostic Pitch Class Scoring, PIN Ring-Buffer Overwrite, Onscreen Backspace, Tap-to-Start Level Switching, iPad 4:3 Layout Math & Kodály Octave Vertical Lines Complete | **Last Updated:** August 1, 2026
...
* **Game Mode:** Features an algorithmic level progression with a manual Level Browser and celestial "Musical Constellation" progress unlocks.
...
* **Architecture:** Master/Worker separation. `main.lua` (Director & State Machine) manages worker modules (`ui`, `playback`, `engine`, `progression`, `definitions`). Workers operate independently to prevent circular dependency crashes.
...
- **`main.lua` (Director & State Machine)**: Manages app state (`idle`, `quiz`, `result`), listens for hardware/keyboard/touch input, orchestrates timers, evaluates user submissions via agnostic pitch class matching, and manages session scoring.
...
* **PIN Cursor-Pointer Ring-Buffer Overwriting:**
    ...
    * Tapping a key when the answer buffer is full overwrites the active cursor slot and advances to the next slot (e.g. for a 3-note box A, B, C: typing `1 2 3` gives A=1, B=2, C=3; typing `4` gives A=4, B=2, C=3; typing `5` gives A=4, B=5, C=3). Single-note exercises immediately overwrite slot 1.
...
* **Master Level Architecture Reconciliation:** Restored full granular level definitions from v12 Bible and enforced strict 1-chromatic max limits for Level 6.x and Level 10.x.
* **Archive Policy Enforced:** Saved historical Bible v13.2 to `zzzz archives/The Solfège Trainer Bible (v13.2).md`.
* **Agnostic Pitch Class Scoring & Theoretical Auto-Correction:** Correct pitch classes award 100% full credit while feedback auto-corrects to proper theory spelling.
* **PIN Cursor-Pointer Overwriting:** Implemented exact slot wrap-around cursor model ($1 \to 2 \to 3 \to \text{wrap to } 1$).
* **Onscreen `⌫ del` Touch Button:** Relocated to right side of answer boxes directly above `↵ submit` button.
