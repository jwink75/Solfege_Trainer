# The Solfège Star Bible (v15.0)
**Status:** Production - User Telemetry & Multi-Profile Architecture Complete | Attribute-Tagged Logging, Mastery Index Algorithm, Local JSON Storage & Profile Switching | **Last Updated:** August 2, 2026
...
### 4. Local Persistence Format (`solfege_star_profiles.json`)
Profiles are saved locally to `system.DocumentsDirectory` as a single structured JSON document supporting multiple student profiles.

***

**August 2 Release Notes (v15.0):**
* **Master Version Upgrade (v15.0):** Created master Bible v15.0 incorporating full User Telemetry Architecture.
* **Archive Policy Enforced:** Saved historical Bible v14.0 to `zzzz archives/The Solfège Star Bible (v14.0).md`.
* **Telemetry & Profile Engine (`stats.lua`):** Implemented multi-user profile persistence, attribute-tagged event logging, and the unified Mastery Index algorithm.
* **Incidental Tendency Detector:** Added `engine.detectTendencies(notes)` to automatically detect and tag tendencies occurring within random melodies.
* **Symmetrical Action Button Pairs:** Positioned `play key` / `question` (left edge) and `⌫ del` / `↵ submit` (far right edge) with identical $86 \times 38$ dimensions, vertically centered on the answer plane (`screenOriginY + screenH * 0.38`).
