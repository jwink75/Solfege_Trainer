...
- **Sign In Modal (`widget.newScrollView`)**: Features an integrated `widget.newScrollView` container holding all registered student profiles. Supports **smooth Macbook two-finger trackpad gestures**, **mouse scrollwheels** (via single unified `"mouse"` listener inspecting `event.scrollY` and direction flags, eliminating mouse-Y coordinate conflict), **click-and-drag mouse dragging** (via `scrollView:takeFocus(event)`), and **touch drag on mobile**. Allows infinite profile expansion (2 to 50+ users) with vertical scrolling, while keeping the modal height capped (`maxCardH = 380px`) and the **`+ New User`** button anchored fixed at the bottom of the card.
